...
void display_array( char * array, int row, int col){
	int i = 0;
	while(i <= row * col){
		cout << setw(60) << "     |     |     " << endl;
...
int get_user_input( char * array, char symbol){
	int input;
...
bool is_there_a_winner( char * array, int row, int col){
...
bool is_it_a_draw( char * array, int row, int col ){
	int count = 0;
	for( int i=0; i < row * col; i++){
...
int main(){
	...
	while( playAgain == 'y'){
		...
		do{
			...
			if( user_turn == true){
				playermove = get_user_input( &moves[0][0], playermark );
				update_array( playermove, playermark, &moves[0][0] );
				user_turn = false;		// After player move it is toggled to false so in next loop
			}							// its computer's turn. 
			else{
				display_countdown( compmark );
				compmove = get_comp_input( &moves[0][0] );
				update_array( compmove, compmark, &moves[0][0] );
			...
			display_array( &moves[0][0], 3, 3);		// Displays the updated grid.
			
			cout << "   _______________________________________________ \n" << endl;
			
			if( is_there_a_winner( &moves[0][0], 3, 3) == true)
 				break;
			
			game_finished = is_it_a_draw( &moves[0][0], 3, 3);
